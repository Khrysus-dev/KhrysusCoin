...
// The zKHRS block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zKHRS is a hash of the serial
...
{
    //Create an output returning the zKHRS that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZKHRSOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zKHRS output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zKHRS", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZKHRSOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zKHRS output", __func__);
...
{
    CzKHRSTracker* zkhrsTracker = pwallet->zkhrsTracker.get();
    CMintMeta meta;
    if (!zkhrsTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zkhrsTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
...
{
    //The unique identifier for a KHRS stake is the outpoint
