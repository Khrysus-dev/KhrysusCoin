...
public:
    ...
    bool WriteZKHRSSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZKHRSSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZKHRSSeed_deprecated(uint256& seed);
    bool EraseZKHRSSeed();
    bool EraseZKHRSSeed_deprecated();

    bool WriteZKHRSCount(const uint32_t& nCount);
    bool ReadZKHRSCount(uint32_t& nCount);
