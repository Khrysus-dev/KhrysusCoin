...
CzKHRSWallet::CzKHRSWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zkhrs seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZKHRSSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZKHRSSeed_deprecated()) {
                    LogPrintf("%s: Updated zKHRS seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zkhrs seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zkhrs wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzKHRSWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZKHRSCount(nCountLastUsed);
    else if (!walletdb.ReadZKHRSCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzKHRSWallet::Lock()
{
    seedMaster = 0;
}

void CzKHRSWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzKHRSWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZKHRS(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzKHRSWallet::LoadMintPoolFromDB()
{
...
void CzKHRSWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzKHRSWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzKHRSWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zkhrsTracker->HasPubcoinHash(pMint.first)) {
...
bool CzKHRSWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZKHRS(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zkhrsTracker which also adds to database
    pwalletMain->zkhrsTracker->Add(dMint, true);
...
void CzKHRSWallet::SeedToZKHRS(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzKHRSWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzKHRSWallet::GenerateDeterministicZKHRS(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzKHRSWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzKHRSWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzKHRSWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
