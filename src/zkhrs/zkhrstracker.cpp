...
CzKHRSTracker::CzKHRSTracker(std::string strWalletFile)
{
...
CzKHRSTracker::~CzKHRSTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzKHRSTracker::Init()
{
...
bool CzKHRSTracker::Archive(CMintMeta& meta)
{
...
bool CzKHRSTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzKHRSTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzKHRSTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzKHRSTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzKHRSTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzKHRSTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzKHRSTracker::GetSerialHashes()
{
...
CAmount CzKHRSTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzKHRSTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzKHRSTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzKHRSTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzKHRSTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzKHRSTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzKHRSTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzKHRSTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzKHRSTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzKHRSTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzKHRSTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzKHRSWallet* zKHRSWallet)
{
    bool iszKHRSWalletInitialized = (NULL != zKHRSWallet);
    CMintMeta meta;
    ...
    if (! iszKHRSWalletInitialized)
        zKHRSWallet = new CzKHRSWallet(strWalletFile);
    meta.isSeedCorrect = zKHRSWallet->CheckSeed(dMint);
    if (! iszKHRSWalletInitialized)
        delete zKHRSWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzKHRSTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzKHRSTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzKHRSTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzKHRSTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzKHRSTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzKHRSTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzKHRSWallet* zKHRSWallet = new CzKHRSWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zKHRSWallet);
        }
        delete zKHRSWallet;
        LogPrint("zero", "%s: added %d dzkhrs from DB\n", __func__, listDeterministicDB.size());
    }
