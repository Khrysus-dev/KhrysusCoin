...
/**
 * Khrysus RPC command dispatcher.
 */
class CRPCTable
{
private:
...
extern UniValue DoZkhrsSpend(const CAmount nAmount, bool fMintChange, bool fMinimizeChange, vector<CZerocoinMint>& vMintsSelected, std::string address_str, bool isPublicSpend = true);
...
extern UniValue setzkhrsseed(const UniValue& params, bool fHelp);
extern UniValue getzkhrsseed(const UniValue& params, bool fHelp);
extern UniValue generatemintlist(const UniValue& params, bool fHelp);
extern UniValue searchdzkhrs(const UniValue& params, bool fHelp);
extern UniValue dzkhrsstate(const UniValue& params, bool fHelp);
