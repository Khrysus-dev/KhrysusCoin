...
#include "ui_zkhrscontroldialog.h"
...
void ZPivControlDialog::updateList()
{
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        {
            LOCK(pwalletMain->zkhrsTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zkhrsTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zKHRS.";
            else if (!mint.isSeedCorrect)
                strReason = "The zKHRS seed used to mint this zKHRS is not the same as currently hold in the wallet";
            else
