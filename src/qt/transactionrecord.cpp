...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet* wallet, const CWalletTx& wtx)
{
    ...
    if (wtx.HasZerocoinSpendInputs()) {
        ...
        if (wtx.HasZerocoinPublicSpendInputs()) {
            ...
            if (!ZKHRSModule::ParseZerocoinPublicSpend(wtx.vin[0], wtx, state, publicSpend)){
    ...
    if (wtx.IsCoinStake()) {
        ...
        if (wtx.HasZerocoinSpendInputs() && (fZSpendFromMe || wallet->zkhrsTracker->HasMintTx(hash))) {
            //zKHRS stake reward
            sub.involvesWatchAddress = false;
            sub.type = TransactionRecord::StakeZKHRS;
...
bool IsZKHRSType(TransactionRecord::Type type)
{
    switch (type) {
        case TransactionRecord::StakeZKHRS:
...
void TransactionRecord::updateStatus(const CWalletTx& wtx)
{
    ...
    else if (type == TransactionRecord::Generated || type == TransactionRecord::StakeMint || type == TransactionRecord::StakeZKHRS || type == TransactionRecord::MNReward) {
